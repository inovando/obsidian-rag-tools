...
function loadVectorStore(targetDir) {
  ...
  if (!fs.existsSync(indexPath)) return { version: '1.0.0', updatedAt: null, chunks: [] };
  try {
    const raw = fs.readFileSync(indexPath, 'utf8');
    return JSON.parse(raw);
  } catch (e) {
    return { version: '1.0.0', updatedAt: null, chunks: [] };
  }
}

/**
 * Salva o índice vetorial em .obsidian/rag-index.json
 */
 ...
 * Suporta filtro de conjunto de caminhos permitidos (allowedPathsSet) para tags/topics estritos.
 */
...
/**
 * Atualiza incrementalmente uma única nota no índice vetorial.
 */
  ...
  try {
    ...
    if (content && content.trim()) {
      ...
      for (const chunk of chunks) {
        const embedding = await generateEmbedding(chunk.textToEmbed);
        if (embedding) {
          store.chunks.push({
