...
function recordValidationError(category = 'content') {
  const m = load();
  m.validationErrorsPrevented = (m.validationErrorsPrevented || 0) + 1;
  if (category === 'format') {
    m.formatErrorsPrevented = (m.formatErrorsPrevented || 0) + 1;
  } else {
    m.contentRulesPrevented = (m.contentRulesPrevented || 0) + 1;
  }
  save(m);
}
...
function getMetrics() {
  ...
  return {
    ...
    validationErrorsPrevented: m.validationErrorsPrevented || 0,
    formatErrorsPrevented: m.formatErrorsPrevented || 0,
    contentRulesPrevented: m.contentRulesPrevented || 0,
