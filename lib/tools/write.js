...
function validateContent(content, filePath) {
  ...
  if (lines[0].trim() !== '---') { errors.push('Missing YAML frontmatter'); return { errors, warnings }; }
  const endIdx = lines.findIndex((l, i) => i > 0 && l.trim() === '---');
  if (endIdx === -1) { errors.push('Missing YAML frontmatter closing'); return { errors, warnings }; }

  const fmText = lines.slice(1, endIdx).join('\n');
  let fm = {};
  try { fm = yaml.load(fmText) || {}; }
  catch (e) { errors.push('YAML syntax error: ' + e.message); return { errors, warnings }; }
  ...
  if (/\bTODO\b/.test(bodyText)) errors.push("contains placeholder: 'TODO'");
  if (/\bTBD\b/.test(bodyText)) errors.push("contains placeholder: 'TBD'");

  // Verifica wiki-links pendentes para gerar aviso (sem bloquear a escrita)
  ...
  while ((m = wikiLinkRegex.exec(cleanBody)) !== null) {
    ...
    if (!vaultFiles.has(resolved) && !target.startsWith('http')) {
      warnings.push("Wiki-link target not found (pending note): [[" + target + "]]");
    }
  }
...
async function handleWriteNote(args) {
  ...
  const { errors, warnings } = validateContent(finalContent, relPath);
  if (errors.length > 0) {
    const hasFormat = errors.some(e => e.includes('line limit') || e.includes('frontmatter') || e.includes('YAML') || e.includes('must be'));
    recordValidationError(hasFormat ? 'format' : 'content');
  ...
  return { success: true, lineCount, filePath: relPath, warnings };
}
