...
function handleReadNote(args) {
  ...
  if (summaryOnly) {
    ...
    // B5: Linhas são 1-indexed sobre o CORPO (pós-frontmatter), não o raw.
    // O frontmatter já é enviado separadamente pelo formatToolResult.
    const s = Math.max(1, startLine || 1);
    const e = Math.min(bodyLines.length, endLine || bodyLines.length);
    finalContent = bodyLines.slice(s - 1, e).join('\n');
