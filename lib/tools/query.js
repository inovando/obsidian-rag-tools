...
async function handleQueryKnowledgeBase(args) {
  ...
  // R2b: Verificar idade do índice vetorial e avisar se desatualizado
  let indexWarning = null;
  try {
    const indexPath = path.join(targetDir, '.obsidian', 'rag-index.json');
    if (fs.existsSync(indexPath)) {
      const stat = fs.statSync(indexPath);
      const ageHours = (Date.now() - stat.mtimeMs) / (1000 * 60 * 60);
      if (ageHours > 24) {
        indexWarning = `⚠️ O índice vetorial tem ${Math.round(ageHours)}h de idade. Use reindex_vault para atualizar.`;
      }
    } else {
      indexWarning = '⚠️ Índice vetorial não encontrado. Use reindex_vault para criar.';
    }
  } catch (e) {
    // Ignore - não bloqueia a busca
  }
