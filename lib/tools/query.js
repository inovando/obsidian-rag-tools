...
/**
 * Identifica se uma nota é uma Nota Âncora canônica do projeto.
 */
function isAnchorNote(note) {
  if (!note) return false;
  const relPath = normalizeRelPath(note.filePath || '');
  const basename = path.basename(relPath).toLowerCase();
  if (basename === '_project.md' || basename === '_anchor.md' || basename === 'overview.md') {
    return true;
  }
  const fm = note.frontmatter || {};
  if (fm.is_anchor === true) return true;
  const tags = (fm.tags || []).map(t => String(t).toLowerCase());
  if (tags.some(t => t === 'meta/project' || t === 'anchor')) return true;
  return false;
}

/**
 * Pré-filtro por tags, tópicos e escopo de projeto.
 */
function filterNotes(notes, tags, topic, project) {
  const hasTags = tags && Array.isArray(tags) && tags.length > 0;
  const hasTopic = topic && typeof topic === 'string' && topic.trim().length > 0;
  const hasProject = project && typeof project === 'string' && project.trim().length > 0;

  if (!hasTags && !hasTopic && !hasProject) return notes;

  const reqProjectBare = hasProject ? project.toLowerCase().trim().replace(/^proj-/, '') : null;

  return notes.filter(n => {
    const fm = n.frontmatter || {};
    const relPath = normalizeRelPath(n.filePath);
    const noteTags = (fm.tags || []).map(t => typeof t === 'string' ? t.toLowerCase() : '');

    if (hasProject) {
      const matchFolder = relPath.startsWith(`proj-${reqProjectBare}/`) ||
                          relPath.startsWith(`projects/${reqProjectBare}/`) ||
                          relPath.startsWith(`${reqProjectBare}/`);
      const matchTag = noteTags.some(t => t === `proj-${reqProjectBare}` || t === reqProjectBare || t.includes(reqProjectBare));
      if (!matchFolder && !matchTag) return false;
    }

    if (hasTags) {
      const hasMatchingTag = tags.some(reqTag => {
        const lowerReq = reqTag.toLowerCase().trim();
        const reqBare = lowerReq.replace(/^proj-/, '');
        return noteTags.some(t => {
          const tBare = t.replace(/^proj-/, '');
          return t === lowerReq || tBare === reqBare || t.includes(lowerReq) || lowerReq.includes(t);
        });
      });
...
async function handleQueryKnowledgeBase(args) {
  ...
  // Auto-sincroniza alterações em disco antes de ler as notas
  try {
    const { syncIndexWithDisk } = require('../rag/vectorStore');
    await syncIndexWithDisk(targetDir);
  } catch (e) {
    // Ignore
  }

  const hasFilter = (tags && tags.length > 0) || (topic && topic.length > 0) || (project && project.length > 0);
  const allNotes = getAllNotes();
  const filteredNotes = filterNotes(allNotes, tags, topic, project);
  ...
  // 3. Reciprocal Rank Fusion (N6: K = 20) com Anchor Note Boosting (N5)
  const combinedMap = new Map();
  const K = 20;
  const allCandidatePaths = new Set([...keywordRanks.keys(), ...vectorRanks.keys()]);

  // N1 Fallback: Se houver filtro de tag/tópico/projeto e a query zerou candidatos, inclui notas do filtro
  ...
  allCandidatePaths.forEach(filePath => {
    ...
    let rrfScore = (0.6 * rrfVec) + (0.4 * rrfKw);

    const noteObj = filteredNotes.find(n => normalizeRelPath(n.filePath) === filePath);

    if (noteObj) {
      // N5: Boost de 40% para nota âncora do projeto
      if (isAnchorNote(noteObj)) {
        rrfScore *= 1.4;
      }
  ...
  if (!hasFilter) {
    indexWarning = "💡 DICA DE USO: Esta busca foi feita sem filtro de tag ou projeto. Para máxima precisão em vaults com múltiplos projetos, passe project: '<slug>' ou tags: ['<tag>']. Use 'list_projects' para listar todos os projetos.";
  }
