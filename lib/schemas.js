...
const tools = [
  {
    ...
    description: "Busca notas de referência no vault Obsidian combinando busca semântica por similaridade vetorial (RAG local) e filtro por palavras-chave/tags/tópicos (RRF). RECOMENDADO: Passe 'tags' ou 'project' para máxima precisão em vaults com múltiplos projetos. Auto-sincroniza alterações recentes em disco.",
    inputSchema: {
      type: "object",
      properties: {
        query: {
          type: "string",
          description: "Texto da consulta livre ou pergunta conceitual (ex: 'Como funciona a reconciliação do Godot?')."
        },
        project: {
          type: "string",
          description: "Nome ou slug do projeto para restringir a busca (ex: 'cicatriz-eterna', 'webapp', 'rimatur'). Altamente recomendado em vaults multiprojeto."
        },
        tags: {
          type: "array",
          items: { type: "string" },
          description: "Filtro por lista de tags (ex: ['cicatriz-eterna', 'godot']). RECOMENDADO em vaults com múltiplos projetos para garantir 100% de precisão."
        },
        topic: {
          type: "string",
  ...
  {
    name: "list_projects",
    description: "Lista todos os projetos cadastrados no vault com suas notas-âncora, contagem de notas e tags. Ferramenta essencial para agentes descobrirem o mapa do vault ao iniciar uma nova sessão.",
    inputSchema: {
      type: "object",
      properties: {
        targetDir: {
          type: "string",
          description: "Diretório opcional do vault."
        }
      }
    }
  },
  ...
  },
  {
    ...
    description: "Gerencia a memória contínua de sessão (.obsidian/session_memory.json) com isolamento por projeto. RECOMENDADO: passe o parâmetro 'project' (ex: 'cicatriz-eterna') para evitar sobrescrever o contexto de outros projetos no mesmo vault.",
    inputSchema: {
      type: "object",
      properties: {
        action: {
        ...
        project: {
          type: "string",
          description: "Nome ou slug do projeto (ex: 'cicatriz-eterna', 'webapp', 'rimatur'). RECOMENDADO para manter contextos isolados por projeto."
        },
        context: {
          type: "string",
  ...
  {
    name: "reindex_vault",
    description: "Sincroniza/reindexa as notas no índice vetorial (.obsidian/rag-index.json). Por padrão realiza sincronização incremental ultrarrápida dos arquivos alterados em disco. Passe force: true para reconstrução completa.",
    inputSchema: {
      type: "object",
      properties: {
        targetDir: { type: "string" },
        force: { type: "boolean", description: "Se true, força a reindexação completa de todo o vault. Padrão é false (sincronização incremental rápida)." }
      }
    }
  },
  {
